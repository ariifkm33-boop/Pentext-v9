...
def cb(c):
    ...
    try:
        ...
        if not c.message:
            bot.answer_callback_query(c.id, "Old message", show_alert=False)
            return
        ...
        if d == 'm':
            ...
            if av <= 0:
                u = db.get_user(uid)
                need = 2 - ((u['refer_count'] if u else 0) % 2)
                if need == 2: need = 0
            ...
            if total == 0:
                try: bot.edit_message_text('No links yet', uid, mid, reply_markup=mk_kb())
                except: pass
                bot.answer_callback_query(c.id)
                return
